...
export async function GET(req: NextRequest) {
  try {
    ...
    const concluidas = lista.filter((v: any) => v.status === 'concluida');
    const canceladas = lista.filter((v: any) => v.status === 'cancelada');
    const pendentes  = lista.filter((v: any) => v.status === 'pendente');
    const faturamento = concluidas.reduce((s: number, v: any) => s + (v.valor_final ?? 0), 0);
    const ticketMedio = concluidas.length ? faturamento / concluidas.length : 0;

    // Faturamento por dia
    const porDia: Record<string, number> = {};
    concluidas.forEach((v: any) => {
